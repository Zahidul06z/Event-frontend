...
const AddProduct = () => {
    ...
    const [course,setCourse] = useState({title : '',price : '',category : '',stock :'',description : ''})

    const [error,setError] = useState('')

    const {title,price,category,stock,description} = course
    ...
    const handleSubmit = async(e) =>{
      ...
      try {
        const res = await fetch(`/api/products/create/${user?._id}`,{
          method: "POST",
          headers: {
      ...
      setCourse({title : '',price : '',category : '',stock :'',description : ''})
  ...
  return (
    <div className='min-h-screen mt-10 p-2 flex justify-center bg-indigo-900 w-full md:mt-20'>
      <ToastContainer 
        ...
        <form className='border-1 border-white py-2 px-2 mb-10 shadow-2xl sm:px-4' onSubmit={handleSubmit}>
            <div>
                <div className='flex justify-center border-1 border-white mt-4 mb-6'>
                  <button onClick={handleClick} className='text-white font-semibold my-2 cursor-pointer '>Upload Product Image </button>
           ...
           <div>
                {previews?.image && <div className='relative'>
                  <img src={previews.image} className='w-[320px] h-[400px]  rounded-[5px] shadow-lg'  />
           ...
           <div className='mb-6'>
            <input type="text" className='bg-blue-100 w-[320px] px-2 py-1.5 text-blue-700 outline-none ' name='title' id='title' placeholder='Title...' onChange={handleChange} value={title} />
           </div>
            <div className='mb-6'>
              <input type="number" className='bg-blue-100 w-[320px] px-2 py-1.5 text-blue-700 outline-none ' name='price' id='price' placeholder='Price...' onChange={handleChange} value={price} />
           </div>
           <div className='mb-6'>
              <input type="text" className='bg-blue-100 w-[320px] px-2 py-1.5 text-blue-700 outline-none ' name='category' id='category' placeholder='Category (eg : Oil,Gee,Ice Cream...)' onChange={handleChange} value={category} />
           </div>
           <div className='mb-6'>
              <input type="number" className='bg-blue-100 w-[320px] px-2 py-1.5 text-blue-700 outline-none ' name='stock' id='stock' placeholder='Stock(Total Number Of Product)...' onChange={handleChange} value={stock} />
           </div>
           <div className='mb-6'>
              <textarea className='bg-blue-100 text-blue-700 w-[320px] h-[100px] outline-none px-2'  name="description" id="description" placeholder='Description...' onChange={handleChange} value={description} />
           </div>
