...
import useGetCodProduct from '../hooks/useGetCodProduct';
...
export default function SuccessPage() {
  ...
  const {orderProduct,loading,error} = useGetCodProduct(id)
...
return (
    <div className="bg-white p-6  my-40 w-full">
      <div className="text-center">
        ...
        <h2 className="mt-4 text-xl font-bold text-green-600 sm:text-2xl">Order Successful!</h2>

        {error && <p className="text-red-500 mt-4">{error}</p>}

          {/* Loading */}
          {loading && <div className="flex justify-center items-center h-full">
            <div className="w-20 h-20 border-2 border-green-600 border-t-transparent rounded-full animate-spin"></div>
          </div>}
          ...
          {orderProduct && 
            ...
            <div className='flex flex-col items-center gap-3'>
              ...
              <h3 className='text-[12px] font-semibold sm:text-xl'>Your order number ID :  {orderProduct?._id}</h3>
            ...
            <div>
              ...
              <div>
                ...
                {orderProduct?.items && Object.values(orderProduct?.items).map((item,index)=>{
              ...
              <div className='flex justify-between bg-gray-100 py-2 px-3'>
                <h4 className=' font-semibold'>Shipping {orderProduct?.shippingCost === 100 ? 'Barisal' : 'Outside Barisal'}</h4>
                <div className=' flex'>
                  <TbCurrencyTaka className='text-[20px]' />
                  <h3 className=' font-semibold text-[15px]'>{orderProduct?.shippingCost}</h3>
                </div>
              </div>
              ...
              <div className='flex justify-between mb-7 bg-gray-100 px-3 py-2 mt-5'>
                ...
                <div className=' flex'>
                  ...
                  <h3 className=' font-semibold text-xl'>{orderProduct?.totalAmount}</h3>
                </div>
              </div>
                ...
                <div>
                  <h3>{orderProduct?.customerName}</h3>
                </div>
                <div>
                  <h3>Cash On Delivery</h3>
                  <h3>{orderProduct?.shippingAddress?.delivaryAddress}</h3>
                  <h3>{orderProduct?.shippingAddress?.city}</h3>
                  <h3>{orderProduct?.shippingAddress?.postCode}</h3>
                  <h3>{orderProduct?.shippingAddress?.country}</h3>
                  <h3>{orderProduct?.phone}</h3>
                  {/* <h3>Cash On Delivery</h3> */}
                </div>
              </div>
            ...
            <div>
              ...
              <p className="font-semibold">{new Date(orderProduct.createdAt).toLocaleString()}</p>
            </div>
          </div>
          }
